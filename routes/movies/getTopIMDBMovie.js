...
const getTopIMDB = async (fastify) => {
    fastify.get('/api/movies/top-imdb', async (request, reply) => {
        let { page } = request.query
        if (!page) {
            page = 1
        }
        const req = await fetch(`https://flixhq.to/top-imdb?type=movie&page=${page}`)
